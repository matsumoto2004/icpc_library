...
int main() {
    ...
    while(1){
        ...
        while(ok-ng>1){
            ll mid=(ok+ng)/2;
            if(mid*mid*mid>=n) ok=mid;
            else ng=mid;
        }
        auto whe=lower_bound(all(yaku),ok)-yaku.begin();

        ll ans=linf;

        while(whe<yaku.size()){
            ll now=yaku[whe];
            vl yaku2=enum_div(n/now);


            ll div1=*lower_bound(all(yaku2),mysqrt(n/now));
            ll div2=(n/now)/div1;

            if(div1<=now){
                chmin(ans,now+div1+div2);
            }
            whe++;
        }
        cout << ans << endl;
