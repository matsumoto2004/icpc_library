...
void solve3(ll n,vl v){
    vector<vector<bitset<1060>>> dp(n+5,vector<bitset<1060>>(1060));
    ...
    rep(j,big){
        ...
        rep(k,big){
            if(dp[n][j][k]){//kビット目
                ll num2=k;
                ll num3=sum-num1-num2;
                chmax(ans,min(num1,min(num2,num3)));
            }
        }
    }
    cout << ans << endl;
