...
int main() {
    ...
    vector<mint> cnt0(n,0),cnt2(n,0);

    rep(i,n)rep(pos,n){
        ll mn=max(0LL,pos-k),mx=min(n-1,pos+k);
        ll range=mx-mn;//+1しない
        cnt0[i]+=comb(n-range-1,i)*comb.fact[n-i-1]*comb.fact[i];
    }

    rep(i,n)rep(range,k,2*k){
        if(i<=1) continue;
        mint basyo=n-2-range+1;
        mint x=comb(n-range-2,i-2);
        x*=comb.fact[i]*comb.fact[n-i-1];
        x*=basyo;
        x*=2*k-range;
