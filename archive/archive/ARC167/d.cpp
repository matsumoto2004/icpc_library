...
public:
    ...
    dsu(int n) : n(n), par(n, -1), rank(n, 0),mex(n,0),last(n,0) {}
...
void solve(){
    ...
    rep(i,n){
        seen[v[i]]++;
        while(seen[allmex]>=1) allmex++;

        //print(uf.mex);

        if(uf.mex[uf.root(i)]<v[i]){
            
            ll j=rev[uf.mex[uf.root(i)]];
            //print(i,j,1);//
            uf.merge(i,j);
            swap(v[i],v[j]);
            rev[v[i]]=i;
        ...
        else if(uf.last[uf.root(i)]==i&&i!=n-1){
            //print("ok");
