...
int main() {
    ...
    while(tk.update()){
        ...
        while(x2-x1>=2||y2-y1>=2){
            border--;
            ll mid=50;
            if(sum1[x]+have<0) mid+=25;
            if(sum1[y]+have<0) mid-=25;
            if(sum1[x]+have>400) mid+=25;
            if(sum1[y]+have>400) mid-=25;

            if((y2-y1!=0&&random(0,99)>=mid)||(x2-x1==0)){
                if(left){
                    while(y<y2-1){
                        set_or_get();
                        move(1);
        ...
        if(!remains.empty()&&remains.back().first<x) reverse(all(remains));
