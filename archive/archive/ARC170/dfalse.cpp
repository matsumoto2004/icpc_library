...
void solve(){
    ...
    rep(i,n){
        ...
        ll nowa=a[i+1];
        {
           ll ue=nowa+b[1];
           ll sita=nowa-b[1];
           if(a[i+2]>=ue&&a[i]<=sita) nowans=false; 
        }
        /*
        rep(iii,1){
            ll mx=a[i+2]-nowa;//mx以下をぶつけないといけない
            ll whe=upper_bound(all(b),mx)-b.begin()-1;
            if(whe==0) continue;
            ll val=b[whe];
            if(val<nowa) continue;
            if(a[i]<=val-nowa) nowans=false; 
