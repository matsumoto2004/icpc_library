...
int main() {
    ...
    mint ans=0;
    //print(cnt);
    rep(i,k+1)rep(j,k+1){//ab,ac
        if(i+j>cnt[0]) continue;
        for(ll ii=0;ii<=i+j;ii++)for(ll jj=0;jj<=k;jj++){//ba,bc
            
            ll iii=i+j-ii,jjj=ii+jj-i;//ca,cb

            //出ていく量チェック
            if(ii+jj>cnt[1]) continue;
            if(iii<0||jjj<0) continue;
            if(iii+jjj>cnt[2]) continue;

            if(j+jj!=iii+jjj) continue;
            ...
            pls*=comb(cnt[0]-i,j);
            pls*=comb(cnt[1],ii);
            pls*=comb(cnt[1]-ii,jj);
            pls*=comb(cnt[2],iii);
            pls*=comb(cnt[2]-iii,jjj);
            ans+=pls;
        }
    }
    cout << ans << endl;
