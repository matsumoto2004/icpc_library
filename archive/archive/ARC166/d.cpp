...
int main() {
    ...
    while(ng-ok>1){
        ...
        lazy_segtree<M> sg(n+5);
        rep(i,n+2) sg.set(i,y[i]);

        bool isok=true;

        rep(i,1,n+1){
            ll right=lower_bound(all(x),x[i]+mid+1)-x.begin()-1;
            ll left=lower_bound(all(x),x[i]-mid)-x.begin();
            if(right==i||left==i) continue;

            ll req=sg.get(i);
            ll cnt1=sg.prod(left,i);
            sg.apply(left,i+1,max(-req,-cnt1));
            req-=cnt1;
            if(req<=0) continue;
            ll cnt2=sg.prod(i+1,right+1);
            if(req-cnt2>0){
                isok=false;
                break;
            }
            sg.apply(i,right+1,-req);
        }
        if(isok) ok=mid;
        else ng=mid;
    }
