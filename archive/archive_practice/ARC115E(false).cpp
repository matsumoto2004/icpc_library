...
public:
    ...
    op(const S &l, const S &r) {
        return {l.range+r.range,l.val+r.val};
    }
    ...
    mapping(const F &f, const S &x) {
        return {x.range,x.val+x.range*f};
    }
};
...
int main() {
    ...
    vl range(sz);
    range[0]=cp.rev[0];
    rep(i,1,sz) range[i]=cp.rev[i]-cp.rev[i-1];

    lazy_segtree<M> seg(sz+5);
    rep(i,sz) {
        seg.set({range[i],0});
    }

    //1番目
    seg.apply(0,cp.mp[v[i]]+1,1);
