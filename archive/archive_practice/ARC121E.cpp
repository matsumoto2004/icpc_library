...
int main() {
    ...
    auto dfs=[&](auto dfs,ll v,ll p=-1) -> void {
        ...
        for(ll u:g[v]){
            dfs(dfs,u,v);
            ll n1=n_dp.size(),n2=dp[u].size();
            vm nn_dp(n1+n2-1);
            rep(i,n1)rep(j,n2){
                nn_dp[i+j]+=n_dp[i]*dp[u][j];
            }
            swap(n_dp,nn_dp);
        ...
        vm nn_dp(n_dp.size()+1);
        rep(i,n_dp.size()){
            nn_dp[i]+=n_dp[i];
            nn_dp[i+1]+=n_dp[i]*(sz[v]-1-i);
        }
    ...
    rep(i,dp[0].size()){
        if(i%2==0) ans+=dp[0][i]*comb.fact[n-i];
        else ans-=dp[0][i]*comb.fact[n-i];
    }
    cout << ans << endl;
}