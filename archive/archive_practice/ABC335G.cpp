...
int main() {
    ll n, p;
    cin >> n >> p;
    auto mp=prime<ll>(p-1).factor_list();

    map<ll,ll> mp2;
    //Bを直接求めず，p-1の各々の素因数が残るかをx乗したときに1になるかでみている
    rep(ni,n){
        ll a;
        cin >> a;
        ll g=1;
    ...
    vl pp,cnt;
    for(auto [v,w]:mp2) pp.pb(v),cnt.pb(w);
    
    ll ans=0;

    rep(i,pp.size())rep(j,i){
        if(pp[i]%pp[j]==0) ans+=cnt[i]*cnt[j];
    }
    rep(i,pp.size()) ans+=cnt[i]*cnt[i];
