...
int main() {
    ...
    rep(i,n) cin >> a[i];
    rep(i,n) cin >> b[i];
    vvm beki(big+5,vm(big+5));
    vvm bekis(big+5,vm(big+5));
    vvm dp(n+5,vm(c+big+5));
    //print("ok");

    rep(i,1,big)rep(j,big){
        beki[i][j]=mint(i).pow(j);
    }
    rep(i,big)rep(j,big){
        bekis[i+1][j]=bekis[i][j]+beki[i][j];
    }//縦に塁積和を取ってる

    //1人目
    rep(i,c+1){
        dp[0][i]=bekis[b[0]+1][i]-bekis[a[0]][i];
    }
    ...
    rep(i,n-1){
        rep(j,c+1){
            rep(k,c+1){
                if(j+k>c) continue;
                dp[i+1][j+k]+=dp[i][j]*(bekis[b[i+1]+1][k]-bekis[a[i+1]][k]);
            }
        }
    }
