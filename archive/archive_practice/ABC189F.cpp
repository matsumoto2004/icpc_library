...
int main() {
    ...
    segtree<M> seg1(n+2e5),seg2(n+2e5);//p*seg1+seg2
    rrep(i,n){
        if(out[i]){
            seg1.set(i,1);
            seg2.set(i,0);
            ...
            ld now1=seg1.prod(i+1,i+m+1);
            now1/=m;
            ld now2=seg2.prod(i+1,i+m+1);
            now2/=m;
            now2+=1;
            seg1.set(i,now1);
            seg2.set(i,now2);
    ...
    if(abs(1-seg1.get(0))<numeric_limits<double>::epsilon()){
        print(-1);
        return 0;
    }
    ld ans=seg2.get(0);
