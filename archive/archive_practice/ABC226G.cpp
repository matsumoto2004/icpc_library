...
bool solve(){
    ...
    //3の処理
    {
        ...
        a[2]-=mns;b[2]-=mns;
        ll mns2=min(a[2],b[4]);
        a[2]-=mns2;b[4]-=mns2;b[1]+=mns2;
        if(a[2]>b[3]) return false;
        b[3]-=a[2];b[0]+=a[2];a[2]=0;
    }
    //2の処理
    ll cnt2=b[1]+b[2]+2*b[3]+2*b[4];
    if(a[1]>cnt2) return false;

    //1の処理
    ll sum=b[0]+2*b[1]+3*b[2]+4*b[3]+5*b[4];
    sum-=a[1]*2;
