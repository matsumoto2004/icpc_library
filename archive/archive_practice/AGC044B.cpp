...
int main() {
    ...
    rep(q,n*n){
        ...
        while(!dq.empty()){
            auto [x,y]=dq.front();
            dq.pop_front();
            ll cost=dist[x][y]+exist[x][y];
            rep(to,4){
                ll tox=x+dx[to],toy=y+dy[to];
                if(!isok(tox,toy,n,n)) continue;
                if(dist[tox][toy]<=cost) continue;

                dist[tox][toy]=cost;
                if(exist[tox][toy]) dq.push_back({tox,toy});
