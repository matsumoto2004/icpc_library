...
int main() {
    ...
    if(n==k){
        rep(i,n) cout << all << endl;
        return 0;
    }

    segtree<M2> seg1(n);
    rep(i,k){
        seg.apply(0,v[i],mint(v[i]-1-seg1.prod(0,v[i]))*comb.fact[n-2-i]*comb.ifact[n-k-1]);
        seg.apply(v[i],n,mint(v[i]-seg1.prod(0,v[i]))*comb.fact[n-2-i]*comb.ifact[n-k-1]);
        seg1.set(v[i],1);
        ans[v[i]]=seg.get(v[i]).val();
    }
    seg.apply(0,n,1);//ぴったり同じ

    rep(i,n){
        if(ans[i]==-1) ans[i]=seg.get(i).val();
    }
    rep(i,n) cout << all-ans[i] << endl;
