...
int main() {
    ...
    //1-indexed
    rep(i,n)rep(j,n)rep(k,1<<(2*x+1)){
        //追加する数字
        ll next=(k>>1);
        for(ll l=max(0LL,i-x+1);l<=min(n-1,i+x-1);l++){
            ll pos=l-(i-x+1);
            if((next>>pos)&1) continue;
            dp[i+1][j+1][next|(1<<pos)]+=dp[i][j][k];
        }
        //先に決めない
