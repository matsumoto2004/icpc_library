...
ll solve(vl v0,vl v1){
    trie<2,'0'> tr;
    for(auto x:v0) tr.insert(b2string(x));

    ll ret=linf;
    for(auto x:v1) chmin(ret,tr.query(b2string(x)));
    return ret;
}
...
int main() {
    ...
    rrep(bt,30){
        ...
        for(auto vv:vs){
            ...
            for(auto x:vv){
                if((x>>bt)&1) n_v0.pb(x%(1<<bt));
                else n_v1.pb(x%(1<<bt));
            }

            if(n_v0.size()%2==1){
                chmax(ans,(1<<bt)+solve(n_v0,n_v1));
                ok=false;
                //print(bt);
            }
            else{
                if(!n_v0.empty()) n_vs.pb(n_v0);
                if(!n_v1.empty()) n_vs.pb(n_v1);
            }
