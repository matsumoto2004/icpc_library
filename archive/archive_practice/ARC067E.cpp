...
int main() {
    ll n,a,b,c,d;
    cin >> n >> a >> b >> c >> d;
    vector<mint> dp(n+2000,0);
    dp[0]=1;
    for(int ii=b;ii>=a;ii--){
        vector<mint> new_dp(n+2000,0);
        rep(i,n+1){
            mint now=dp[i];
        
            for(int jj=c;jj<=d&&(i+ii*jj<=n);jj++){
                new_dp[i+ii*jj]+=now*comb(n-i,ii*jj)*comb.fact[ii*jj]*comb.ifact[ii].pow(jj)*comb.ifact[jj];
            }
            new_dp[i]+=dp[i];
        }
        swap(dp,new_dp);
