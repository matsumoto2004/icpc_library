...
void solve(){
    ...
    rep(i,n) cin >> l[i] >> r[i];
    rep(i,n) l[i]--,r[i]--;
    vvl dp(sz+5,vl(sz+5,0));
    for(ll range=1;range<=sz;range++){
        for(ll i=0;i+range<=sz;i++){
            ll j=i+range;//[i,j)
            vl seen(sz+5,0);
            rep(ii,n){
                if(i>l[ii]||j<r[ii]) continue;
                ll gru=dp[i][l[ii]]^dp[r[ii]][j];
                seen[gru]++;
            }
            rep(ii,sz+5){
                if(seen[ii]==0){
