...
int main(){
    ...
    dinic<ll> din(hcnt+wcnt+20);
    ll s=hcnt+wcnt,g=s+1;

    rep(i,hcnt) din.add_edge(s,i,1);
    rep(j,wcnt) din.add_edge(hcnt+j,g,1);

    rep(i,h)rep(j,w){
        if(v[i][j]=='#') continue;
        din.add_edge(id1[i][j],hcnt+id2[i][j],1);
    }

    ll ans=din.max_flow(s,g);
    cout << ans << endl;   
}