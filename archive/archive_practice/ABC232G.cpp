...
int main() {
    ...
    exist=zip(exist);
    map<ll,ll> rev_exist;
    rep(i,exist.size()) rev_exist[exist[i]]=i;

    //graph 0~n-1と0~m-1のうち存在するもの
    vve g(n+exist.size());
    rep(i,exist.size()){
        ll from=i,to=(i+1)%exist.size();
        g[n+from].pb({n+to,md(exist[to]-exist[from])});
    }
    rep(i,n){
        g[i].pb({n+rev_exist[md(-a[i])],0});
        g[n+rev_exist[b[i]]].pb({i,0});
    }
