...
int main() {
    ...
    rep(i,n-1){
        rep(j,m+1){
            rep(k,j,m+1){
                //i番目まで埋めて、[j,k]の範囲にいる状態
                //i+1番目への遷移
                mint now=dp[i][j][k];
                if(mp[i+1]!=-1){
                    if(mp[i+1]<j){
                        dp[i+1][mp[i+1]][k]+=now;
                        dp[i+1][mp[i+1]][k+1]-=now;
                        dp[i+1][mp[i+1]+1][k]-=now;
                        dp[i+1][mp[i+1]+1][k+1]+=now;
                    }
                    else if(mp[i+1]>=k){
                        dp[i+1][j][mp[i+1]+1]+=now;
                        dp[i+1][j][mp[i+1]+1+1]-=now;
                        dp[i+1][j+1][mp[i+1]+1]-=now;
                        dp[i+1][j+1][mp[i+1]+1+1]+=now;
                    }
                    else{
                        dp[i+1][j][k]+=now;
                        dp[i+1][j][k+1]-=now;
    ...
    mint ans=0;
    rep(j,m+1)rep(k,m+1) ans+=dp[n-1][j][k];
    cout << ans << endl;
}
