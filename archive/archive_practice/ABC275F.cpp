...
int main() {
    ...
    vl v(n);
    rep(i,n) cin >> v[i];
    vvl dp1(n+5,vl(m+5,linf)),dp2(n+5,vl(m+5,linf));
    dp1[0][0]=0;
    rep(i,n)rep(j,m+1){
        ll now=v[i];
        chmin(dp2[i+1][j],dp1[i][j]+1);
        chmin(dp2[i+1][j],dp2[i][j]);
        if(j+now<=m){
            chmin(dp1[i+1][j+now],dp2[i][j]);
            chmin(dp1[i+1][j+now],dp1[i][j]);
        }
    }
    rep(i,m){
        ll ans=min(dp1[n][i+1],dp2[n][i+1]);
        if(ans==linf) ans=-1;
        cout << ans << endl;
    }
}