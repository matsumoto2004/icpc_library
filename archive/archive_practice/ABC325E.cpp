...
int main() {
    ...
    cin >> n >> a >> b >> c;
    vvl vd(n,vl(n));
    rep(i,n)rep(j,n) cin >> vd[i][j];
    priority_queue<LP,vector<LP>,greater<LP>> pq;
    vl dis(2*n,linf);//n足すと自家用車に
    //dis[0]=0;
    pq.push({0,0});
    while(!pq.empty()){
        auto p=pq.top();
        ll d=p.first,whe=p.second;
        //print(d,whe);
        pq.pop();
        if(dis[whe]<=d) continue;
        dis[whe]=d;
        
        rep(i,n){
            ll dd=d+b*vd[whe%n][i]+c;
            if(dis[n+i]<=dd) continue;
            pq.push({dd,n+i});
        }
        if(whe>=n) continue;
        rep(i,n){
            ll dd=d+a*vd[whe%n][i];
            if(dis[i]<=dd) continue;
            pq.push({dd,i});
        }
    }
