...
vl solve(vl v,ll k){
    ...
    vl ret;
    segtree<M> seg(n+3e5);
    rep(i,n) seg.set(i,{v[i],i});

    ll now=0;//次見はじめる場所
    ll rest=k;
    while(now<n){
        auto p=seg.prod(now,now+rest+1);
        ret.pb(p.first);
        rest-=(p.second-now);
        now=p.second+1;
...
vl solve2(vl v,ll k,ll kk){
    ...
    while(now<n){
        ll r=now+rest;
        if(now<kk) r=kk+rest;
        auto p=seg.prod(now,r+1);
        ret.pb(p.first);
        if(now>=kk) rest-=(p.second-now);
        else if(p.second>kk) rest-=(p.second-kk); 
        now=p.second+1;
...
int main() {
    ...
    vl newv;
    rep(i,n) newv.pb(v[(whe+i)%n]);
    vl ans2=solve2(newv,k-cnt,cnt);

    vl ans=min(ans1,ans2);
