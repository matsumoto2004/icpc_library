...
int main() {
    ...
    vector<string> v(n);
    rep(i,n) cin >> v[i];
    rep(di,1,n+1)rep(dj,1,n+1){
        rep(i,n)rep(j,n){
            ll ii=i+di,jj=j+dj;
            if(ii>n||jj>n) continue;
            dp[i][j][ii][jj]=max(di,dj);
            if(di==1&&dj==1){
                if(v[i][j]=='.') dp[i][j][ii][jj]=0;
            }

            //縦に割る
            rep(mi,i+1,ii){
                chmin(dp[i][j][ii][jj],dp[i][j][mi][jj]+dp[mi][j][ii][jj]);
            }
            //横に割る
            rep(mj,j+1,jj){
                chmin(dp[i][j][ii][jj],dp[i][j][ii][mj]+dp[i][mj][ii][jj]);
            }
