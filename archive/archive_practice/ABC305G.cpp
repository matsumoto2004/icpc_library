...
int main() {
    ...
    mat mt(64,vector<mint>(64));
    mat first(64,vector<mint>(1));
    vector<bool> canuse(64);
    rep(bit,64){
        string s;
        rep(i,6){
            if((bit>>i)&1){
        ...
        if(ok) canuse[bit]=true;
        else canuse[bit]=false;
    }
    rep(bit,64){
        if(canuse[bit]) first[bit][0]=1;
    }
    rep(bit,64){
        if(!canuse[bit]) continue;
        ll next1=(bit<<1)%64,next2=next1+1;
        if(canuse[next1]) mt[next1][bit]=1;
        if(canuse[next2]) mt[next2][bit]=1;
    }
    mat last=mt.pow(n-6)*first;
    mint ans=0;
    rep(i,64) ans+=last[i][0];
    cout << ans << endl;
