...
int main() {
    ...
    //それ以外
    rep(range,2,n+1)rep(i,n){
        ll j=i+range;
        if(j>n) continue;
        //色のかぶらない2つをmerge
        rep(k,i+1,j){
            dp[i][j][2]+=dp[i][k][1]*dp[k][j][1]*2;
            dp[i][j][3]+=dp[i][k][2]*dp[k][j][1]*3;
            //dp[i][j][3]+=dp[i][k][1]*dp[k][j][2]*3;
        }
        //色がかぶるやつを右に付け足す
        rep(k,1,4){
        ...
        if(mp[{i,j}]!=0){
            rep(k,1,4){
                if(mp[{i,j}]!=k) dp[i][j][k]=0; 
            }
        }
    }
    mint ans=dp[0][n][1]*3+dp[0][n][2]*3+dp[0][n][3];
    cout << ans << endl;
}