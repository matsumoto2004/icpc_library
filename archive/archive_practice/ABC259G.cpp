...
int main() {
    ...
    rep(i,h){
        //sとi or iとg
        if(sum1[i]>0){
            din.add_edge(s,i,sum1[i]);
            ans+=sum1[i];
        }
        if(sum1[i]<0){
            din.add_edge(i,g,-sum1[i]);
            //ans+=-sum1[i];
        }
    }
    rep(j,w){
        ...
        //選ばないと選ぶが逆
        if(sum2[j]>0){
            din.add_edge(h+j,g,sum2[j]);
            ans+=sum2[j];
        }
        if(sum2[j]<0){
            din.add_edge(s,h+j,-sum2[j]);
            //ans+=-sum2[j];
        }
    }
    //print(ans);
    rep(i,h)rep(j,w){
        //i,h+j
        if(v[i][j]>0) din.add_edge(i,h+j,v[i][j]);
        if(v[i][j]<0) din.add_edge(i,h+j,linf);
    }
    //print(sum1,sum2);
    cout << ans-din.max_flow(s,g) << endl;
}