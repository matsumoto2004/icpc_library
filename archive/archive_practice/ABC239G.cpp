...
class dinic {
    struct edge {
        ...
        edge(int to, T cap, int rev) : to(to), cap(cap), rev(rev),id(-1) {}
...
public:
    ...
    void add_edge(int from, int to, T cap,int id) {
        G[from].eb(to, cap, G[to].size(),id);
        G[to].eb(from, 0, G[from].size() - 1);
    }
...
int main() {
    ...
    rep(i,m){
        ...
        cin >> a >> b;
        a--;b--;
        din.add_edge(n+a,b,linf);
        din.add_edge(n+b,a,linf);
    }
    vl vc(n);
    rep(i,n) cin >> vc[i];

    rep(i,1,n-1){
        din.add_edge(i,n+i,vc[i],i);
    }
    //print("ok");
    cout << din.max_flow(n,n-1) << endl;

    vb on=din.on_s(n);
    vl list;
    rep(i,n) if(on[i]&&(!on[n+i])) list.pb(i);
    cout << list.size() << endl;

    for(auto &x:list) cout << x+1 << endl;
