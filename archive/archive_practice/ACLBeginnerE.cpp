...
struct Mypair{
    ...
    ll sz;
    constexpr Mypair(){}
    constexpr Mypair(mint x,ll sz):x(x),sz(sz) {}
    constexpr Mypair(const Mypair &mp){
        x=mp.x;
        sz=mp.sz;
    }
};
...
public:
    ...
    op(const S &l, const S &r) {
        return Mypair(l.x*mint(10).pow(r.sz)+r.x,l.sz+r.sz);
    }
    ...
    mapping(const F &f, const S &x) {
        if(f==-1) return x;
        return Mypair(one[x.sz-1]*f,x.sz);
    }
};
