...
int main() {
    ...
    while(!pq.empty()){
        auto p=pq.top();
        ll num=p.first,whe=p.second;
        pq.pop();
        if(dist[whe]<num) continue;
        ll x=topair(num).first,y=topair(num).second;
        
        for(auto [next,cost]:g[whe]){
            ll nextnum;
            if(y+cost<=vx[x]) nextnum=toll(x,y+cost);
            else{
