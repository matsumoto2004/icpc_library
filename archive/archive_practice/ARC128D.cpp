...
int main() {
    ...
    rep(i,1,n){
        if(v[i-1]==v[i]){
            seg.apply(i,i+1,seg.prod(i-1,i).val);
            seg.apply(0,i,0);
            last=i;
        }
        else if(i==1){
            ...
            mint val=seg.prod(i-2,i).val;
            val+=seg.prod(0,last).val;
            seg.apply(i,i+1,val);
        }
        else{
            mint val=seg.prod(0,i).val;
            seg.apply(i,i+1,val);
            last=i-1;
        }

        debug(i,seg.prod(i,i+1).val);

    }
    cout << seg.prod(n-1,n).val << endl;
