...
int main() {
    ...
    //最初
    dp[(1<<0)][0]=abs(a[0]-b[0]);
    rep(i,1,n){
        dp[(1<<i)][i]=abs(a[i]-b[0]);//+cしなくてよさそう
    }
    //2番目以降
    rep(bit,1,(1<<n))rep(j,n){
        if(!((bit>>j)&1)) continue;
        ll now=dp[bit][j];
        ll cnt=__builtin_popcountll(bit);
        rep(k,n){
            if((bit>>k)&1) continue;
            if(k==j+1) chmin(dp[bit^(1<<k)][k],now+abs(a[k]-b[cnt]));
            else chmin(dp[bit^(1<<k)][k],now+abs(a[k]-b[cnt])+c);
        }
    }
    ll ans=linf;
