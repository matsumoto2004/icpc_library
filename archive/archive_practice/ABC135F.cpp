...
int main() {
    ...
    cin >> s >> t;
    string scopy=s;
    while(s.size()<1000005) s+=scopy;
    string s2=s+s;
    string s3=s+s+s;
    string tt=t;
    while(tt.size()<4800025) tt+=t;
    vector<ll> v1=z_algorithm(tt+s2),v2=z_algorithm(tt+s3);
    ll ns=s.size();
    ll nt=t.size();
    ll ans=0;
    ll ans2=0;
    ...
    rep(i,2*ns){
        chmax(ans,v1[tt.size()+i]);
    }
    rep(i,3*ns){
        chmax(ans2,v2[tt.size()+i]);
    }

    ans/=nt;ans2/=nt;

    //print(ans,ans2);
    if(ans2>ans) cout << -1 << endl;
    else cout << ans << endl;
}