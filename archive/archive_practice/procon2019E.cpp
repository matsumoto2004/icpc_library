...
int main() {
    ...
    cin >> n >> m;
    vvl vv(n,vl(m));
    rep(i,n)rep(j,m) cin >> vv[i][j];
    Matrix<modint<2>> v(n,m);
    rep(i,n)rep(j,m) v[i][j]=vv[i][j];
    ll rk=gaussianEliminationModp<modint<2>>(v);
    mint ans=0;
    mint all=mint(2).pow(m);
    mint dame=mint(2).pow(m-rk);
    ans=mint(2).pow(n-1)*(all-dame);
    cout << ans << endl;
    //print(rk);
}