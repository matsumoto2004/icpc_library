...
public:
    ...
    composition(const F &g, const F &f) {
        if(f==LP{-1,-1}) return g;
        if(g==LP{-1,-1}) return f;
        else return g;
    }
    ...
    mapping(const F &f, const S &x) {
        if(f==LP{-1,-1}) return x;
        if(x.x==-1) return x;
        return {max(x.x,f.second-(x.r-1)),x.l,x.r};
    }
};
...
int main() {
    ll h,w;
    cin >> h >> w;
    lazy_segtree<M> seg(w+5);
    rep(i,w) seg.set(i,{0,i,i+1});
    rep(i,h){
        ll x,y;
        cin >> x >> y;
        x--;y--;
        ...
        while(ok1-ng1>1){
            ll mid=(ok1+ng1)/2;
            if(seg.get(mid).x+mid>=x) ok1=mid;
            else ng1=mid;
            //print(mid,seg.get(mid).x);
        }
        ll ok2=w,ng2=-1;
        while(ok2-ng2>1){
            ll mid=(ok2+ng2)/2;
            if(seg.get(mid).x+mid>=y+1) ok2=mid;
            else ng2=mid;
            //print(mid,seg.get(mid).x);
        }
        if(y+1!=w) seg.apply(ok1,ok2,{y+1,y+1});
        else seg.apply(ok1,ok2,{y+1,linf});
        ll ans=seg.prod(0,w).x+i+1;
        if(ans>ll(inf)) ans=-1;
        cout << ans << endl;
        //print(ok1,ok2);
        //rep(i,w) cout << seg.get(i).x << " ";
        //cout << endl;
        //rep(i,w) cout << seg.prod(0,i+1).x << " ";
        //cout << endl;
    }
    //rep(i,w) cout << seg.get(i).x << " ";
    //cout << endl;
