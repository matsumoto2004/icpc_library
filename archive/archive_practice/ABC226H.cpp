...
int main() {
    ...
    rep(x,100){
        //[x,x+1]の積分
        vector<fps> dp(n+2);//自分より大きいものがi個ある確率
        dp[0]={1};
        rep(i,n){
            vector<fps> ndp(n+2);

            if(r[i]<=x){
                rep(j,n+1) ndp[j]=dp[j];
            }
            else if(l[i]>=x+1){
                rep(j,n+1) ndp[j+1]=dp[j];
            }
            else{
                rep(j,n+1){
                    fps p1={-l[i],1};
                    fps p2={r[i],-1};
                    ndp[j]+=dp[j]*p1/(r[i]-l[i]);
                    ndp[j+1]+=dp[j]*p2/(r[i]-l[i]);
                }
            }
        ...
        fps fin;
        rep(i,k,n+1) fin+=dp[i];
        fin=fin.integral();
        ans+=fin.eval(x+1)-fin.eval(x);
    }
    cout << ans << endl;
