...
int main() {
    ...
    vl v(n);
    rep(i,n) cin >> v[i];
    CartesianTree<ll> ct(v);
    
    vl left(n,-1),right(n,-1);
    rep(i,n) left[i]=i,right[i]=i;
    ...
    vvm rui(2,vm(n+5,0));//ジグザグに累積和を取る
    vvm dp(2,vm(n+5,0));//pls,mns
    vvm dpsum(2,vm(n+5,0));//1indexed
    ...
    //2番目以降
    rep(i,1,n){
        mint val0=dpsum[0][i]-dpsum[(i+1+left[i])%2][left[i]];
        mint val1=dpsum[1][i]-dpsum[(i+0+left[i])%2][left[i]];
        dp[0][i]+=val0*v[i];
        dp[1][i]+=val1*v[i];
        
        rui[1][i+1]+=val0*v[i];
        rui[(i+1+right[i])%2][ct.right[i]+1]-=val0*v[i];

        rui[0][i+1]+=val1*v[i];
        rui[(i+right[i])%2][ct.right[i]+1]-=val1*v[i];

        dp[0][i]+=rui[0][i];
        dp[1][i]+=rui[1][i];

        dpsum[0][i+1]=dpsum[1][i]+dp[1][i];
        dpsum[1][i+1]=dpsum[0][i]+dp[0][i];
