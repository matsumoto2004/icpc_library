...
int main() {    
    ...
    rep(i,n)rep(j,m){
        if(v[i][j]=='o'){
            mcf.add_edge(s,id(i,j),1,big*(i+j));
            cnt++;
        }
        mcf.add_edge(id(i,j),g,1,big*(n+m-i-j));
    }

    //隣接するマス
    rep(i,n)rep(j,m){
        if(i!=n-1&&v[i][j]!='#'&&v[i+1][j]!='#') mcf.add_edge(id(i,j),id(i+1,j),linf,big-1);
        if(j!=m-1&&v[i][j]!='#'&&v[i][j+1]!='#') mcf.add_edge(id(i,j),id(i,j+1),linf,big-1);
    }
    
    vector<LP> ans=mcf.slope(s,g,cnt);

    //print(ans);
    cout << -ans.back().second+cnt*big*(n+m) << endl;
}