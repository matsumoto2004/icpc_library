...
bool query1(ll x){
    ...
    else if(s=="Blue") return false;
    else exit(0);
}

ll query2(ll x){
    ll whe=-1;
    for(auto y:lis[0]) if(x==y) whe=1;
    for(auto y:lis[1]) if(x==y) whe=0;
    if(whe==-1) return -1;

    cout << "? ";
    cout << x+1 << " ";
    for(auto y:lis[whe]) cout << y+1 << " ";
    cout << endl; 
...
int main() {
    ...
    while(ng-ok>1){
        ll mid=(ok+ng)/2;
        if(query1(mid)==zero) ok=mid;
        else ng=mid;
    }
    ans[ok]=zero;
    ans[ok+n]=(!zero);
    rep(i,n-1) lis[0].pb(ng+i);
    rep(i,n-1) lis[1].pb((ng+i+n)%(2*n));
    rep(i,2*n){
        ll a=query2(i);
        if(a==1) ans[i]=1;
        if(a==0) ans[i]=0;
