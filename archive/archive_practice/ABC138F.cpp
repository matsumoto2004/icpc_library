...
int main() {
    ...
    while(now<=r){
        if(now*2-1>=l){
        ll ue=min(r-now,now-1);//大きい方がue以下を数え上げ
        ll sita=max(l-now,0LL); //小さい方がsita以上を数え上げ

        vector<mint> dp(cnt+5);
        vector<mint> dp1(cnt+5);//ueだけギリ
        vector<mint> dp2(cnt+5);//sitaだけギリ
        vector<mint> dp3(cnt+5);
        dp[0]=1;
