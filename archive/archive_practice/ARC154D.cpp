...
void Mergesort(vl &a,ll left,ll right){
    ...
    for (int i = left; i < right; ++i) {
        if(iterator_right==iterator_left){
            a[i]=buf[iterator_left];
            break;
        }
        // 左側採用
        if (query(buf[iterator_left],one,buf[iterator_right])==false) {
            a[i] = buf[iterator_left++];
        }
        // 右側採用
        else {
...
int main() {
    ...
    vl ans;
    rep(i,n) if(i!=one)ans.pb(i);
    Mergesort(ans,0,n-1);
    vl fans(n);
    fans[one]=0;
    rep(i,n-1) fans[ans[i]]=i+1;
    cout << "! ";
    rep(i,n) cout << fans[i]+1 << " ";
    cout << endl;
