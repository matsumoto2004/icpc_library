...
struct D {
    ...
    static constexpr T id = LP{-linf,0LL};//上に余る，余らなくてもいい
    
    static T merge(const T &a, const T &b) {
        return {max(a.first+b.second,b.first+a.second),a.second+b.second};
    }
    
    // u : root, p : parent of u
    static T add_root(const T &a, [[maybe_unused]] int u, [[maybe_unused]] int p) {
        return {a.second,max(a.second,a.first+1)};
    }
};
