...
int main() {
    ...
    mint half=mint(1)/2;
    rep(i,n-1){
        vp1[i]=comb(i,b-1)*half.pow(i+1);//黒がなくなる
        vp2[i]=comb(i,w-1)*half.pow(i+1);//白がなくなる
    }
    vm rui1(n+5),rui2(n+5);//1indexed
    rep(i,n-1){
        rui1[i+1]=rui1[i]+vp1[i];
        rui2[i+1]=rui2[i]+vp2[i];
    }
    vm ans(n);
    rep(i,n){
        ans[i]+=rui2[i];
        ans[i]+=(mint(1)-rui1[i]-rui2[i])*half;
    }
    rep(i,n){
        cout << ans[i] << endl;
    }
