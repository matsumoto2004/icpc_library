...
int main() {
    ...
    while(now<=r){
        ll res=min(r-now,now-1);//大きい方がres以下を数え上げ
        vector<mint> dp(cnt+5,0);//cnt桁の桁dp
        vector<mint> dp2(cnt+5,0);//cnt桁の桁dp
        dp[0]=1;
        rep(i,cnt){
            ll x=((res>>(cnt-1-i))&1);
            //print(i,x);
    ...
    while(now<=l-1){
        ll res=min(l-1-now,now-1);//小さい方がres以下を数え上げ
        vector<mint> dp(cnt+5,0);//cnt桁の桁dp
        vector<mint> dp2(cnt+5,0);//cnt桁の桁dp
        dp[0]=1;
        rep(i,cnt){
