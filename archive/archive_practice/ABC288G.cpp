...
int main() {
    ...
    queue<ll> que;
    que.push((nn-1)/2);
    seen[(nn-1)/2]=1;//入れるときにチェック
    
    vl ans(nn,0);
    vl p3(n);
    rep(i,n) p3[i]=pow(3LL,i);
    while(!que.empty()){
        ll x=que.front();
        que.pop();
        vl b3(n);
        ...
        rep(i,n){
            ...
            ll nxt2=x-p3[i];
            if(!seen[nxt1]){
                if(v[x]!=v[nxt1]) ans[nn-1-nxt1]=1;
                else ans[nn-1-nxt1]=0;
                seen[nxt1]=1;
                que.push(nxt1);
            }
            if(!seen[nxt2]){
                if(v[x]!=v[nxt2]) ans[nn-1-nxt2]=1;
                else ans[nn-1-nxt2]=0;
                seen[nxt2]=1;
                que.push(nxt2);
            }
        }
