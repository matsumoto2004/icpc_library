...
int main() {
    ...
    vector<vector<mint>> comb(505,vector<mint>(505,0));
    rep(i,505){
        comb[i][0] = 1;
        rep(j,1,i+1){
    ...
    rep(i,1,n+1)rep(j,1,i+1){
        mint now=dp[i][j];
        for(ll pls=1;i+pls<=n;pls++){
            mint next=mint(2).pow(pls*(pls-1)/2);
            next*=(mint(2).pow(j)-1).pow(pls);
            next*=now;
            if(i+pls!=n) next*=comb[n-i-1][pls];
            //print(n-i-1,pls,comb[n-i-1][pls]);
            dp[i+pls][pls]+=next;
        }
    }
    cout << dp[n][1] << endl;
