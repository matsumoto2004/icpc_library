...
int main() {
    ...
    vl vc(n);
    rep(i,n) cin >> vc[i];
    vm dp1(n,0),dp2(n,0),dp3(n,0),dp4(n,0);//期待値
    dp4[0]=1;
    vm prob(n,0);
    rep(i,n) prob[i]=mint(1)/graph[i].size();
    rep(trial,k){
        vm ndp1(n,0),ndp2(n,0),ndp3(n,0),ndp4(n,0);
        rep(i,n){
            mint probsum=0;
            ...
            if(vc[i]==1){
                ...
                ndp3[i]+=ndp2[i]*2+ndp4[i];
                ndp2[i]+=ndp4[i];
            }
        }
        swap(ndp1,dp1),swap(ndp2,dp2),swap(ndp3,dp3),swap(ndp4,dp4);
        //print(dp1),print(dp2),print(dp3),print(dp4);
    }
    mint ans=0;
    rep(i,n) ans+=dp1[i];
    cout << ans << endl;
