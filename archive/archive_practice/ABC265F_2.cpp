...
int main() {
    ...
    vvm dp(d+5,vm(d+5,0));
    dp[0][0]=1;
    vvm sum1(d+5,vm(d+5,0));//差が同じ　を集めた
    rep(i,d+1)rep(j,d+1){
        sum1[i+1][j+1]=sum1[i][j]+dp[i][j];
    }
    vvm sum2(d+5,vm(d+5,0));//和が同じを集めた
    rep(i,d+1)rep(j,d+1){
        sum2[i+1][j+1]+=sum2[i][j+2]+dp[i][j];
    }

    //rep(i,d+1) print(sum1[i]);
    //rep(i,d+1) print(sum2[i]);

    rep(ii,n){
        ll x=abs(a[ii]-b[ii]);
        vvm ndp(d+5,vm(d+5,0));
        vvm nsum1(d+5,vm(d+5,0));
        vvm nsum2(d+5,vm(d+5,0));
        ...
        rep(i,d+1)rep(j,d+1){
            ...
            //sum2から
            {
                ...
                //print(a1,a2,b1,b2);
                ndp[i][j]+=sum2[b1+1][b2+1];
                ndp[i][j]-=sum2[a1][a2+2];
            }
            //sum1から
            {
                ll a1=i-x,a2=j;
                ll b1=i,b2=j-x;
                if(a1>=0) ndp[i][j]+=sum1[a1][a2];//
                if(b2>=0) ndp[i][j]+=sum1[b1][b2];//あえて+1しないことで被りを防ぐ
            }

            
        }
        rep(i,d+1)rep(j,d+1){
            nsum1[i+1][j+1]=nsum1[i][j]+ndp[i][j];
        }
        rep(i,d+1)rep(j,d+1){
            nsum2[i+1][j+1]=nsum2[i][j+2]+ndp[i][j];
        }
        swap(dp,ndp);
        swap(sum1,nsum1);
        swap(sum2,nsum2);
