...
int main() {
    ...
    auto dfs=[&](auto dfs,ll v,ll p=-1) -> void {
        ...
        for(ll u:g[v]){
            ...
            dfs(dfs,u,v);
            //自分を白にする
            vvm n_dp(sz1[v]+sz1[u],vm(sz2[v]+sz2[u]));
            rep(i,1,sz1[v])rep(j,sz2[v]){
                rep(ii,sz1[u])rep(jj,sz2[u]){
                    n_dp[i+ii][j+jj]+=dp[v][i][j]*dp[u][ii][jj];
                }
            }
            sz1[v]+=sz2[u]-1;
            sz2[v]+=sz2[u]-1;
            rep(i,1,sz1[v])rep(j,sz2[v]) dp[v][i][j]=n_dp[i][j];
    ...
    mint ans=0;
    rep(i,n)rep(j,k+1,n+1)rep(ii,n+1){
        mint pls=dp[i][j][ii];
        if(i==0){
            pls/=comb(j+ii,j);
        }
