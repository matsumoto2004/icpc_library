...
int main() {
    ll n,m,q;
    cin >> n >> m >> q;
    TwoSatisfiability sat(n*m);

    auto id=[&](ll x,ll y) -> ll {
        return x*m+y;
    };

    rep(i,n)rep(j,m-1){
        sat.add_if(id(i,j),id(i,j+1));
    }
    ...
    rep(qi,q){
        ...
        rep(i,m){
            ...
            if(j<0){
                ...
                sat.set_true(sat.rev(id(a,i)));
                sat.set_true(sat.rev(id(b,i)));
            }
            else{
                sat.add_or(sat.rev(id(a,i)),sat.rev(id(b,j)));
                sat.add_or(sat.rev(id(b,i)),sat.rev(id(a,j)));
        ...
        rep(i,m){
            ...
            if(j<0){
                ...
                sat.add_or(id(a,i),id(b,j));
                sat.add_or(id(b,i),id(a,j));
            }
    ...
    if(!ans.size()){
        ...
        rep(i,n){
            cout << m-accumulate(ans.begin()+i*m,ans.begin()+(i+1)*m,0LL) << endl;
        }
    }
}