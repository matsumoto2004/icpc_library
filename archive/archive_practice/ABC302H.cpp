...
struct RollbackUnionFind {
  vector<int> data;
  vector<int> data2;//一度触れたことがあるかを管理
  stack<pair<int, int> > history;
  stack<pair<int, int> > history2;
  int inner_snap;
  ...
  ll unite(int x, int y) {
    ...
    history2.emplace(x, data2[x]);
    history2.emplace(y, data2[y]);
    if (x == y){
        if(data2[x]==0){
            data2[x]=1;
            return 1;//初めてつなぐ
    ...
    data[x] += data[y];
    data[y] = x;
    if(data2[x]==1&&data2[y]==1) return -1;
    chmax(data2[x],data2[y]);
    chmax(data2[y],data2[x]);
    return 0;//問題ない
  }
  ...
  void undo() {
    ...
    data2[history2.top().first] = history2.top().second;
    history2.pop();
    data2[history2.top().first] = history2.top().second;
    history2.pop();
  }
