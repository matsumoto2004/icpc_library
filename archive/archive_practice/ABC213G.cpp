...
int main() {
    ...
    rep(bit,1<<n){
        rep(j,n)rep(k,j+1,n){
            if(((bit>>j)&1)&&((bit>>k)&1)&&g[j][k]) cnt[bit]++;
        }
    }
    debug(cnt);
    vm dp(1<<n);//頂点集合が連結となるような辺の選び方
    dp[0]=1;
    rep(i,n) dp[(1<<i)]=1;
    rep(bit,1<<n){
        if(popcount(bit)<=1) continue;
        ll look=0;
        while(((bit>>look)&1)==0) look++;
        dp[bit]+=mint(2).pow(cnt[bit]);
        for(ll bit2=bit;bit2>0;(bit2--,bit2&=bit)){
            if(bit2==bit) continue;
            if(((bit2>>look)&1)==0) continue;
            ll bit3=bit&(~bit2);
            dp[bit]-=mint(2).pow(cnt[bit3])*dp[bit2];
        }
    }
    debug(dp);
    vm ans(n);
    rep(i,1,n){
        rep(bit,1<<n){
            if(((bit>>0)&1)==0||((bit>>i)&1)==0) continue;
            ll bit2=(~bit)&(((1<<n)-1));
            ans[i]+=dp[bit]*mint(2).pow(cnt[bit2]);
        }
    }
