...
ll big=2000000;
vm fac(big+5);//0indexed
vm facsum(big+5);//0indexed
void init(){
    fac[0]=1;
    facsum[0]=1;
    rep(i,big) fac[i+1]=fac[i]*(i+1);
    rep(i,big){
        facsum[i+1]=facsum[i]+fac[i+1];
    }
}
...
int main() {
    ...
    seg.set(0,fac[n]);
    rep(i,1,h){
        ll l=max(0LL,i-d),r=i;
        mint now=seg.prod(l,r);
        now*=facsum[n]-1;
        seg.set(i,now);

        //print(i,now);
    }
    ll l=max(0LL,h-d),r=h;
