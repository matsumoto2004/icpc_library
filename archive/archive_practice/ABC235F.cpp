...
int main() {
    ...
    vvm dp1(n+5,vm(1<<10,0)),dp2(n+5,vm(1<<10,0));//ちょうどor余裕あり
    vvm sum1(n+5,vm(1<<10,0)),sum2(n+5,vm(1<<10,0));
    
    //1文字目
    {
        dp1[0][(1<<(s[0]-'0'))]+=1;
        sum1[0][(1<<(s[0]-'0'))]+=s[0]-'0';
    }
    rep(i,1,s[0]-'0'){
        dp2[0][(1<<i)]+=1;
        sum2[0][(1<<i)]+=i;
    ...
    rep(i,1,n){
        ...
        rep(j,(1<<10)){
            rep(k,x){
                dp2[i][j|(1<<k)]+=dp1[i-1][j];
                sum2[i][j|(1<<k)]+=sum1[i-1][j]*10+dp1[i-1][j]*k;
            }
            dp1[i][j|(1<<x)]+=dp1[i-1][j];
            sum1[i][j|(1<<x)]+=sum1[i-1][j]*10+dp1[i-1][j]*x;
        }
        //dp2から引き続き
        rep(j,(1<<10)){
            rep(k,10){
                dp2[i][j|(1<<k)]+=dp2[i-1][j];
                sum2[i][j|(1<<k)]+=sum2[i-1][j]*10+dp2[i-1][j]*k;
            }
        }
        //新しく作る
        rep(j,1,10){
    ...
    rep(i,(1<<10)){
        ...
        //if(dp1[n-1][i]!=mint(0)) print(i);
        //if(dp2[n-1][i]!=mint(0)) print(i);
    }
