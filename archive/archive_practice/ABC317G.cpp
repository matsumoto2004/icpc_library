...
class dinic {
    struct edge {
        ...
        edge(int to, T cap, int rev) : to(to), cap(cap), rev(rev), used(false) {}
    };
...
public:
    ...
    void show(ll &x,ll &nn,ll &mm,vvl &ans){
        rep(i,nn)rep(j,G[i].size()){
            auto &e=G[i][j];
            if(e.cap==0&&e.used==false&&e.to>=nn&&e.to<2*nn){
                e.used=true;
                G[e.to][e.rev].cap=0;
                ans[i][x]=e.to-nn;
            }
...
int main() {
    ...
    rep(i,n)rep(j,m) v[i][j]--;
    dinic<ll> din(3*n+5);
    ll s=n*2,g=n*2+1;
    rep(i,n)rep(j,m){
        din.add_edge(i,n+v[i][j],1);
    }
    vvl ans(n,vl(m,-1));
    rep(ii,m){
    rep(i,n) din.add_edge(s,i,1);
    rep(i,n) din.add_edge(n+i,g,1);
    assert(din.max_flow(s,g)==n);
    din.show(ii,n,m,ans);
    }
