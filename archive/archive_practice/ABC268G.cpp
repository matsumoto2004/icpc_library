...
struct Node {
    ...
    Node(int c) : next(char_size, -1), accept(0), accept2(0), c(c) {}
};
...
public:
    ...
    int count2(const string &s){
        ...
        rep(i,s.size()){
            ...
            pos=next;
            if(i!=s.size()-1) ret+=nodes[pos].accept;
        }
        return ret;
    }
...
int main() {
    ...
    rep(i,n) cin >> v[i];
    trie<26,'a'> tr;
    rep(i,n) tr.insert(v[i]);
    rep(i,n){
        mint ans=n;
        ans-=tr.count1(v[i]);
        ans+=tr.count2(v[i]);
        //print(tr.count1(v[i]),tr.count2(v[i]));
        ans/=2;
        ans+=1;
        cout << ans << endl;
