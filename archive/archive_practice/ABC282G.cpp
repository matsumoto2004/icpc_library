...
int main() {
    ...
    cin >> n >> k >> p;
    mint::set_mod(p);
    vector<vvm> dp(n+5,vvm(n+5,vm(n+5,0)));
    //１回目
    rep(i,n)rep(j,n){
        dp[0][i][j]=1;//0個similar，A下にi個，B下にj個
    }

    //2回目以降
    rep(update,n-1){
        vector<vvm> newdp(n+5,vvm(n+5,vm(n+5,0)));
        rep(ii,n)rep(i,n)rep(j,n){
            mint now=dp[ii][i][j];
            //down,down
            newdp[ii+1][0][0]+=now;
            newdp[ii+1][i][j]+=now;
            ...
            //down,up
            newdp[ii][0][j]+=now;
            newdp[ii][i][n-update-2+1]+=now;
            newdp[ii][0][n-update-2+1]-=now;
            newdp[ii][i][j]-=now;
            //up,down
            newdp[ii][i][0]+=now;
            newdp[ii][n-update-2+1][j]+=now;
            newdp[ii][i][j]-=now;
            newdp[ii][n-update-2+1][0]-=now;
            //up,up
            newdp[ii+1][i][j]+=now;
            newdp[ii+1][n-update-2+1][n-update-2+1]+=now;
            newdp[ii+1][i][n-update-2+1]-=now;
            newdp[ii+1][n-update-2+1][j]-=now;
        }
        rep(ii,n){
            rep(i,n)rep(j,n) newdp[ii][i][j+1]+=newdp[ii][i][j];
            rep(i,n)rep(j,n) newdp[ii][i+1][j]+=newdp[ii][i][j];
        }
        swap(dp,newdp);
