...
int main() {
    ...
    auto v2=convex_hull(v);

    n=v2.size();
    
    ll sid=-1,gid=-1;
    rep(i,n){
        if(dist(v2[i],point(sx,sy))<0.1) sid=i;
        if(dist(v2[i],point(gx,gy))<0.1) gid=i;
    }

    vector<long double> ans(2);

    if(sid==-1||gid==-1){
        cout << dist(point(sx,sy),point(gx,gy)) << endl;
        return 0;
    }


    rep(ri,2){
        for(ll i=sid;1;i++,i%=n){
            ll ni=(i+1)%n;
            ans[ri]+=dist(v2[i],v2[ni]);
