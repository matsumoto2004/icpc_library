...
int main() {
    ...
    rep(i,n){
        ll x=lower_bound(all(whe[a[i]]),i)-whe[a[i]].begin();
        if(whe[a[i]][x-1]==-1) dp[i]=seg.prod(0,i)+1;
        else{
            dp[i]=seg.prod(whe[a[i]][x-1],i);
            seg.set(whe[a[i]][x-1],0);
        }
        seg.set(i,dp[i]);
    }
