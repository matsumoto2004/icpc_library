...
int main() {
    ...
    rep(i,n){
        if(s[i]=='0'){
            whe.pb(i);
            cnt.pb(i-prev);
            if(!cntsum.empty()) cntsum.pb(cntsum.back()+cnt.back());
            else cntsum.pb(cnt.back());
            prev=i+1;
        }
    }
    //print("ok");
    ll sz=whe.size();
    //dp[0]の更新
    rep(i,cnt[0]+1) dp[0][i][0]=1;
    //配るdp
    rep(i,sz-1)rep(j,301)rep(ii,301){
        rep(next,301){
            if(j+next>cntsum[i+1]) break;
            dp[i+1][j+next][ii+max(0LL,next-cnt[i+1])]+=dp[i][j][ii];
        }
    }
    mint ans=0;
    rep(ii,k+1) ans+=dp[sz-1][cntsum.back()][ii];
