...
bool operator< (const fps &a,const fps &b){
    return false;
}
...
int main() {
    ...
    rep(i,2*n) mp1[v[i]]++;
    vector<int> list;
    for(auto [a,b]:mp1) list.eb(b);
    sort(all(list));
    priority_queue<pair<ll,fps>> kakes;
    for(auto x:list){
        mint now=1;
        fps kake;
        for(int i=0;i*2<=x;i++){
        ...
        kakes.push({-kake.size(),kake});
    }
    while(kakes.size()>1){
        auto [x,tp1]=kakes.top();        
        kakes.pop();
        auto [y,tp2]=kakes.top();
        kakes.pop();
        fps ret=tp1*tp2;
        kakes.push({-ret.size(),ret});
    }
    auto [x,dp]=kakes.top();
    vm cntp(n+1,0);
    cntp[0]=1;
    rep(i,n+1){
        mint now=comb.fact[2*i];
        now/=mint(2).pow(i);
        now*=comb.ifact[i];
        cntp[i]=now;
    }
