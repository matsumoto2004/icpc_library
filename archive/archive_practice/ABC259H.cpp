...
int main() {
    ...
    rep(i,n)rep(j,n) v[i][j]--;
    vector<vector<LP>> num(n*n);
    rep(i,n)rep(j,n) num[v[i][j]].pb(LP{i,j});
    mint ans=0;
    rep(i,n*n){
        if(num[i].size()<=n){
            rep(ii,num[i].size())rep(jj,ii,num[i].size()){
                auto p1=num[i][ii],p2=num[i][jj];
                if(p1>p2) swap(p1,p2);
                if(p1.second>p2.second) continue;
                ans+=comb(p2.second-p1.second+p2.first-p1.first,p2.first-p1.first);
            }
        }
        else{
            vector<vector<mint>> dp(n+5,vm(n+5));
            rep(ii,n)rep(jj,n){
                if(v[ii][jj]==i){
                    dp[ii][jj]+=1;
                    ans+=dp[ii][jj];
