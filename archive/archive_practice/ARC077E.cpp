...
int main() {
    ...
    ll ans=0;
    rep(i,n-1) ans+=(v[i+1]-v[i]+m)%m;
    lazy_segtree<M> seg(2*m+5);//お得度
    rep(i,n-1){
        ll now=(v[i+1]-v[i]+m)%m;
        if(v[i]==v[i+1]) continue;
        if(v[i]<v[i+1]){
            seg.apply(v[i]+2,v[i+1]+1,1);
            seg.apply(v[i+1]+1,v[i+1]+2,-(now-1));            
        }
        else{
            seg.apply(v[i]+2,v[i+1]+m+1,1);
            seg.apply(v[i+1]+m+1,v[i+1]+m+2,-(now-1));    
        }
    }
    ll mns=0;
    ll now=0;
    ...
    decode[0]=seg.get(0);
    rep(i,1,2*m+3){
        decode[i]=decode[i-1]+seg.get(i);
    }
    rep(i,m){
        chmax(mns,decode[i]+decode[i+m]);
