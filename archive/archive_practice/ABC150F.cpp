...
public:
    rolling_hash(vl s):s(s),n(s.size()) {
        init();
    }
...
int main() {
    ...
    rep(i,n) cin >> a[i];
    rep(i,n) cin >> b[i];
    vl ga(n),gb(n);//gbを何個右シフトするとgaになるか，
    rep(i,n) ga[i]=a[i]^a[(i+1)%n];
    rep(i,n) gb[i]=b[i]^b[(i+1)%n]; 
    
    ga.insert(ga.end(),all(ga));
    ...
    auto p=rla.get(gb);
    rep(i,n){
        //print(rla.get(i,i+n),rlb.get(0,n));
        if(rla.get(i,i+n)==p){
