...
int main() {
    ...
    rep(i,n) cin >> v[i];
    sort(all(v));
    vl lower(n,-1),same(n,-1),upper(n,-1);
    lower[0]=0;
    rep(i,1,n){
        if(v[i]!=v[i-1]) lower[i]=i;
        if(v[i]==v[i-1]) lower[i]=lower[i-1];
    }
    upper[n-1]=0;
    rep(i,1,n){
        ll ii=n-1-i;
        if(v[ii]!=v[ii+1]) upper[ii]=i;
        if(v[ii]==v[ii+1]) upper[ii]=upper[ii+1];
    }
    rep(i,n) same[i]=n-1-upper[i]-lower[i];
