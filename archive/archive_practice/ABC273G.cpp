...
int main() {
    ...
    rep(i,n) cin >> r[i];
    rep(i,n) cin >> c[i];
    if(SUM(r)!=SUM(c)) fin(0);

    vvm dp(n+5,vm(n+5,0));
    ll twocnt=0;
    rep(i,n) if(c[i]==2) twocnt++;
    dp[0][twocnt]=1;
    ll nowsum=SUM(r);//これを元に計算

    rep(i,n){
        rep(j,n+1){
            if(dp[i][j].val()==0) continue;
            ll two=j,one=nowsum-2*two;
            ...
            if(r[i]==1){
                dp[i+1][j]+=dp[i][j]*one;
                if(j!=0) dp[i+1][j-1]+=dp[i][j]*two;
            }
            if(r[i]==2){
                if(j!=0) dp[i+1][j-1]+=dp[i][j]*two;
                if(j>1) dp[i+1][j-2]+=dp[i][j]*two*(two-1)/2;
                if(j!=0) dp[i+1][j-1]+=dp[i][j]*two*one;
                dp[i+1][j]+=dp[i][j]*one*(one-1)/2;
            }
        }
        nowsum-=r[i];
    }
