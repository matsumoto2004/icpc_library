...
int main() {
    ...
    mint ans=0;
    mint nowa=0,nowb=0,nowc=0;
    rep(i,a+1) nowa+=comb(n,i);
    rep(i,b+1) nowb+=comb(n,i);
    rep(i,c+1) nowc+=comb(n,i);
    mint half=1;
    half/=2;
    rep(i,n+1){
        mint pls=1;
        ...
        mint aa=comb(n-i-1,a);
        if(a>n-i-1) aa=0;
        nowa=(nowa-aa)*half+aa;

        mint bb=comb(n-i-1,b);
        if(b>n-i-1) bb=0;
        nowb=(nowb-bb)*half+bb;

        mint cc=comb(n-i-1,c);
        if(c>n-i-1) cc=0;
        nowc=(nowc-cc)*half+cc;
    }
    cout << ans << endl;
}