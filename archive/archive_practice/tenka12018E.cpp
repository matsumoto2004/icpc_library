    ...
    rep(rot,4){
        rep(i,h+w-1)rep(j,i+1,h+w-1){
            ...
            rep(x1,305){
                ...
                ll x01=x1-gap-1;
                ll y01=i-x01;
                if(isin(x01,y01,h,w)&&v[x01][y01]=='#') cnt1--;
                //追加対象
                ll x02=x1;
                ll y02=i-x02;
                if(isin(x02,y02,h,w)&&v[x02][y02]=='#') cnt1++;

                ll y1=j-x1;
                if(y1<0||y1>=w) continue;
                ll x2=x1+gap;
                ll y2=j-x2;
                if(!isin(x2,y2,h,w)) continue;
                if(v[x1][y1]=='#'&&v[x2][y2]=='#') ans+=cnt1;
        ...
        rep(i,h){
            rep(j,w){
                if(v[i][j]!='#') continue;
                for(ll ii=2;j+ii<w;ii+=2){
                    ll k=j+ii;
                    //(i,j)(i,k)
                    ll x1=i+ii/2,y1=(j+k)/2;
                    if(!isin(x1,y1,h,w)) break;
                    if(v[i][k]=='#'&&v[x1][y1]=='#') ans--;
                }
            }
        }
        v=rotate(v,h,w);
