...
public:
    ...
    bool is_prime() {
        auto v = factor_list();
        return v.size() == 1 && (*v.begin()).second == 1;
    }
...
int main() {
    ...
    rep(i,n){
        if(a[i]==1){
            ...
            if(a[i]%2==0) din.add_edge(s,i,b[i]);
            else din.add_edge(i,g,b[i]);
        }
    }

    rep(i,n)rep(j,n){
        if(a[i]%2!=0||a[j]%2!=1) continue;
        if(!prime(a[i]+a[j]).is_prime()) continue;
        din.add_edge(i,j,linf);
    }
    
    //1がないならもう答えちゃう
    if(one==-1){
        cout << din.max_flow(s,g) << endl;
        return 0;
    }
    ...
    while(ng-ok>2){
        ...
        ll c2=(ok+ng*2)/3;
        dinic<ll> din1=din;
        din1.add_edge(oneid,g,c1);
        dinic<ll> din2=din;
        din2.add_edge(oneid,g,c2);
        if(din1.max_flow(s,g)+(one-c1)/2>=din2.max_flow(s,g)+(one-c2)/2) ng=c2;
        else ok=c1;
    }
    ll ans=0;
    rep(i,3){
        if(ok>one) continue;
        dinic<ll> din1=din;
        din1.add_edge(oneid,g,ok);
        chmax(ans,din1.max_flow(s,g)+(one-ok)/2);
        ok++;
    }
