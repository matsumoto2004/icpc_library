...
int main() {
    ...
    rep(ni,60){
        ...
        vl cnt(q);
        vector<vector<pair<LP,ll>>> see1(big);//クエリ
        vector<vector<pair<LP,ll>>> see2(big);//クエリ

        rep(qi,q){
            ll mid=(ok[qi]+ng[qi])/2;
            ll x1=max(0LL,a[qi]-mid-1);
            ll x2=min(big-1,a[qi]+mid);
            ll y1=max(0LL,b[qi]-mid);
            ll y2=min(big-1,b[qi]+mid+1);
            if(a[qi]-mid-1>=0)see1[x1].pb({{y1,y2},qi});
            see2[x2].pb({{y1,y2},qi});
            //debug(x1,x2,y1,y2);
        }
        ...
        rep(i,big){
            for(auto j:xtoy[i]) seg.set(j,seg.get(j)+1);
            for(auto [yy,qi]:see1[i]){
                auto [y1,y2]=yy;
                cnt[qi]-=seg.prod(y1,y2);
            }
            for(auto [yy,qi]:see2[i]){
                auto [y1,y2]=yy;
                cnt[qi]+=seg.prod(y1,y2);
            }
        }
