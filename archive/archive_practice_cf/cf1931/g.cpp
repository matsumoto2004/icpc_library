...
void solve(){
    ...
    if(v[0]==v[1]+1){
        mint ans=1;
        ans*=comb(v[0]-1+v[2],v[2]);
        //ans*=mint(v[2]).pow(v[0]);
        ans*=comb(v[0]-1+v[3],v[3]);
        //ans*=mint(v[3]).pow(v[0]);
        cout << ans << endl;
        return;
    }
    if(v[0]==v[1]){
        mint ans1=1;
        ans1*=comb(v[0]+1-1+v[2],v[2]);
        //ans1*=mint(v[2]).pow(v[0]+1);
        ans1*=comb(v[0]-1+v[3],v[3]);
        //ans1*=mint(v[3]).pow(v[0]);
        mint ans2=1;
        ans2*=comb(v[0]-1+v[2],v[2]);
        //ans2*=mint(v[2]).pow(v[0]);
        ans2*=comb(v[0]+1-1+v[3],v[3]);
        //ans2*=mint(v[3]).pow(v[0]+1);
        mint ans=ans1+ans2;
        cout << ans << endl;
        return;
    }
    if(v[0]+1==v[1]){
        mint ans=1;
        ans*=comb(v[1]-1+v[2],v[2]);
        //ans*=mint(v[2]).pow(v[1]);
        ans*=comb(v[1]-1+v[3],v[3]);
        //ans*=mint(v[3]).pow(v[1]);
        cout << ans << endl;
        return;
    }
}
