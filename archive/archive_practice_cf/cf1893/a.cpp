...
public:
    ...
    ll mx(vvi &rev){
        ...
        rep(i,0,num){
            ll mx=-linf;
            bool iscycle=false;
            for(auto x:scc_list[i]){
                for(auto y:rev[x]){
                    if(id[x]==id[y]) iscycle=true;
                    chmax(mx,dp[id[y]]);
                }
            }
            if(mx<0) continue;
            dp[i]=mx+1;
            if(iscycle) dp[i]=linf;
            chmax(ret,dp[i]);
        }
        //print(dp);
        return ret;
...
void solve(){
    ...
    //逆から見ることに注意
    vvi g(n),rev(n);//何回シフトした状態->何回シフトした状態に遷移できるか
    rep(i,n){
        if(v[i]>n) continue;
        ll to=(i+v[i])%n;
        ll x=((v[i]-1-to)%n+n)%n,y=(x+v[i])%n;
        g[x].pb(y);
        rev[y].pb(x);
        //print(x,y);
    }
