...
void solve(){
    ...
    rep(i,n){
        ll xx,yy;
        cin >> xx >> yy;
        if((xx+yy)%2==0) st[0].insert(i);
        else st[1].insert(i);
    }
    if(x==1) swap(st[0],st[1]);
    //for(auto it=st[0].begin();it!=st[0].end();it++) cout << *it << endl;
    //firstなら最後に選ばれるやつをst0の要素にしたい
    //secondならそうしたくない
    if(st[0].size()>=st[1].size()){
        ...
        while(1){
            ...
            cin >> x;
            x--;
            if(st[0].find(x)!=st[0].end()) st[0].erase(x);
            else st[1].erase(x);
            n--;
            if(n==0) break;
        }
