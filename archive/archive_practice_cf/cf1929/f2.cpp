...
void solve(){
    ...
    cin >> n >> c;
    vl l(n),r(n),val(n);
    rep(i,n) cin >> l[i] >> r[i] >> val[i];
    rep(i,n){
        if(l[i]!=-1) l[i]--;
        if(r[i]!=-1) r[i]--;
    }
    vl vmx(n,c),vmn(n,1);//制約条件

    auto dfs=[&](auto dfs,ll v,ll mx,ll mn) -> void {
        chmin(vmx[v],mx);
        chmax(vmn[v],mn);
        ...
        if(r[v]!=-1){
            ...
            ll nmx=mx,nmn=mn;
            if(val[v]!=-1) chmax(nmn,val[v]);
            dfs(dfs,u,nmx,nmn);
        }
    };
    dfs(dfs,0,c,1);

    vl vmx2(n,c),vmn2(n,1);

    auto dfs2=[&](auto dfs2,ll v) -> void {
        if(l[v]!=-1){
            ll u=l[v];
            dfs2(dfs2,u);
            chmin(vmx2[v],vmx2[u]);
            chmax(vmn2[v],vmn2[u]);
            if(val[u]!=-1) chmax(vmn2[v],val[u]);
        }
        //右へ
        if(r[v]!=-1){
            ll u=r[v];
    ...
    rep(i,n){
        if(which[v]==0) chmax(vmn[i],mnmn);
        else chmin(vmx[i],mxmx);
    }
    }
