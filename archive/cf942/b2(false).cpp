...
void solve(){
    ...
    rep(g,1,big){
        vl v=prime(g).enum_divisors();
        ll mxi=a/g,mxj=b/g;
        if(mxi==0||mxj==0) break;
        for(auto x:v){
            ll ue=min(mxi,x-1);
            ll sita=max(1LL,x-mxj);
            memo[g]+=max(0LL,ue-sita+1);

            //debug(g,mxi,mxj,x,ue,sita);
        }
    }
    ...
    rep(i,1,big){
        ...
        vl v=prime(i).enum_divisors();
        
        for(auto j:v){
            if(j!=i) memo[i]-=memo[j];
        }
