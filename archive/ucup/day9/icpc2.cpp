...
int main() {
    ...
    rep(bit,(1<<13)){
        ...
        rep(i,13){
            if((bit>>i)&1) continue;
            ll next=(bit|(1<<i));
            if(y+cnt[i]>w) chmin(dp[next],toll(x+1,cnt[i]));
            else chmin(dp[next],toll(x,y+cnt[i]));
        }
    }
    ll ans=dp[(1<<13)-1];
    if(ans%big==0) ans=ans/big;
    else ans=ans/big+1;
    cout << ans << endl;
}