...
{
    std::uniform_int_distribution<ull> dist(low, high);
    return dist(gen);
}
...
int main() {
    ll big=1000050;
    vector<ull> coef(big);
    vector<ull> coefsum(big+1);
    rep(i,big) coef[i]=random(1,mod-1);
    rep(i,big){
        coefsum[i+1]=calc_mod(coefsum[i]+coef[i]);
    }
    ...
    auto calc=[&](auto calc,ll left,ll right) -> void {
        ...
        //左側が大きい場合,どちらも同じ場合
        {
            ...
            rrep(i,mid,left){
                ...
                if(mn==mx){
                    if(cnt.count(coef[mn])){
                        good[i].pb(cnt[coef[mn]]+1);
                    }
                }
                else{
                    ull other=calc_mod(mod*4+coefsum[mx+1]-coefsum[mn]+coef[mn]-now);
                    if(cnt.count(other)){
                        good[i].pb(cnt[other]+1);
                    }
                }
            }
