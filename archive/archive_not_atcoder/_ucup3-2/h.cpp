...
int main() {
    ...
    long double ok=0,ng=1001;

    rep(ri,40){///変えろ！！！！！！！
        long double mid=(ok+ng)/2;
        vector<long double> nsum(n+1);
        rep(i,n+1) nsum[i]=sum[i]-mid*i;
        ...
        rep(i,1,n){
            auto it=mp.upper_bound(nsum[i]);
            it--;
            mp[nsum[i]]=it->second+1;

            auto it2=mp.upper_bound(nsum[i]);
